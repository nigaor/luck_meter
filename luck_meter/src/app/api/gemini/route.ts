...
export async function POST(request: Request) {
  ...
  try {
    ...
    const response = await genAI.models.generateContent({
      ...
      config: {
        systemInstruction: 
        "あなたはポジティブ心理学の専門家です。以下の出来事をポジティブ度と重要度を考慮して、人が死ぬレベルが-100点、何も起きない通常が0点、人生最大レベルの幸福を+100点とした場合を参考に-100点から+100点で1桁単位で数字が細かくなるように点数化してください。下ネタは0点にしてください。またその点数を付けた理由を3行程度のユーモアのある簡単な日本語の親しみやすく可愛い文章で具体的に出来事になぞらえて評価してください。応答は必ず以下のJSON形式で返してください。\n例: {\"score\": 75, \"comment\": \"新しい発見があり、とても充実した一日でしたね✨\"}",
      },
    });
