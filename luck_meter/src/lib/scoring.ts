...
export const resultScoringFunction: ScoringFunction = async (
  text: string
): Promise<{ score: number; comment: string }> => {
  const aiScoringFunction: ScoringFunction = async (
    text
  ): Promise<{ score: number; comment: string }> => {
    try {
      ...
      if (typeof data.score !== "number" || isNaN(data.score)) {
        throw new Error(
          `AIからの点数解析に失敗しました。: ${JSON.stringify(data)}`
        );
      }
      return {
